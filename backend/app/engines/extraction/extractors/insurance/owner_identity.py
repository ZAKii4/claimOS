...
# Matches "Nom [et prénom] [du propriétaire] : VALUE" style key/value lines,
# stopping at the next label-looking token (a colon-terminated word).
OWNER_NAME_LABEL = re.compile(
    r"nom(?:\s+et\s+pr[ée]nom)?(?:\s+du\s+propri[ée]taire)?\s*[:\-]\s*(.+)",
    re.IGNORECASE,
)


class OwnerNameExtractor(BaseExtractor):
    """Extracts an owner/policy-holder name from key/value style document text."""

    @property
    def name(self) -> str:
        return "insurance.owner_name_extractor"
    ...
    def extract(
        ...
        for block in ocr_result.page.blocks:
            for line in block.lines:
                ...
                match = OWNER_NAME_LABEL.match(text.strip())
                if not match:
                    continue
    ...
    def validate_format(self, value: str) -> bool:
        # Names are alphabetic tokens (allowing hyphens/apostrophes), 2-60 chars.
